...
public class Callback extends LoginbuddyDemoclientCommon {
  ...
  protected void doGet(HttpServletRequest request, HttpServletResponse response)
    ...
    if (code != null) {
      ...
      try {
        HttpPost req = new HttpPost(String.format("%s/token", location_loginbuddy));
      ...
      response.sendRedirect(String.format("democlientCallback.jsp?state=%s&error=%s&error_description=%s", clientState,
          URLEncoder.encode(error, StandardCharsets.UTF_8), URLEncoder.encode(error_description, StandardCharsets.UTF_8)));
    }
  }
